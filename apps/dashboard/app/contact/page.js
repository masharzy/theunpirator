...
const paths = [
  [
    ReceiptText,
    "Plans & pricing",
    "Compare the live public plans configured by the platform admin, including current features and limits.",
    "/pricing",
    "View pricing",
  ],
  [
    BookOpen,
    "Integration help",
    "Start with the developer documentation for architecture, stack examples and playback flow.",
    "/docs",
    "Open docs",
  ],
  [
    LifeBuoy,
    "Existing customers",
    "Use Support inside your workspace so your request stays connected to the correct account and tenant.",
    "/dashboard/support",
    "Open support",
  ],
];
        ...
        <section className="border-b border-[#dce3d5] bg-[#172014] text-white">
          <div className="mx-auto grid max-w-7xl gap-10 px-5 py-20 md:px-8 md:py-24 lg:grid-cols-[1fr_.85fr] lg:items-end">
            ...
            <p className="max-w-xl text-base leading-7 text-[#bbc5b6]">
              We route questions to the place that already has the right context instead of
              publishing a generic contact form that goes nowhere useful.
            </p>
          </div>
        </section>
        ...
        <section className="mx-auto max-w-7xl px-5 py-20 md:px-8 md:py-24">
          <div className="grid gap-5 md:grid-cols-3">
            {paths.map(([Icon, title, body, href, label]) => (
              <article
                key={title}
                className="group rounded-[28px] border border-[#dce3d5] bg-white p-7 shadow-[0_14px_45px_rgba(35,48,28,.05)]"
              >
                ...
                <Link
                  href={href}
                  className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-[#40512f]"
                >
                  {label}{" "}
          ...
          <div className="mx-auto grid max-w-7xl gap-8 px-5 py-16 md:grid-cols-2 md:px-8 md:py-20">
            <div>
              ...
              <h2 className="mt-4 text-3xl font-semibold tracking-[-.04em]">
                Bring the useful context.
              </h2>
            </div>
            <div className="grid gap-3 text-sm text-[#5f6a59] sm:grid-cols-2">
              {[
                "Workspace / site",
                "Affected playbackRef",
                "Approximate time",
                "Browser / device",
                "Relevant error text",
                "Expected behavior",
              ].map((item) => (
                <div key={item} className="rounded-xl bg-[#f3f6ee] px-4 py-3">
                  {item}
                </div>
              ))}
            </div>
