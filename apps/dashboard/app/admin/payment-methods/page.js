...
export default function AdminPaymentMethodsPage() {
  ...
  const load = () =>
    api("/v1/admin/commerce/payment-methods").then((data) => setItems(data.items || []));
  useEffect(() => {
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {items.map((item) => (
            <Surface key={item.id} className="p-5">
              ...
              <p className="mt-2 text-xs text-[#7d8876]">
                {item.accountType || "Account"} · {item.accountName || "No account name"}
              </p>
          ...
          {items.length === 0 && (
            <p className="text-sm text-[#7b8574]">No manual payment method configured.</p>
          )}
        </div>
      </div>
    </div>
