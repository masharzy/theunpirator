...
function DetailRow({ label, value, mono = false }) {
  return (
    <div className="grid gap-1 border-b border-[#edf0e9] py-3 last:border-0 sm:grid-cols-[140px_1fr] sm:gap-4">
      ...
      <dd className={`min-w-0 break-words text-sm text-[#25301f] ${mono ? "font-mono text-[12px]" : ""}`}>
        {value ?? "—"}
      </dd>
    </div>
  );
      ...
      <aside className="absolute inset-y-0 right-0 flex w-full max-w-2xl flex-col border-l border-[#dde2d8] bg-[#f7f8f4] shadow-2xl">
        <div className="flex items-start gap-4 border-b border-[#e0e4dc] bg-white px-5 py-5 sm:px-6">
          <div className="min-w-0 flex-1">
            <p className="text-[10px] font-bold uppercase tracking-[.18em] text-[#7b856f]">Security event</p>
            <h2 className="mt-2 truncate text-xl font-semibold tracking-[-.02em] text-[#20291c]">
              {event ? cleanType(event.type) : "Loading event…"}
            </h2>
            {event && (
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6">
          {error ? (
            ...
            <div className="space-y-4">
              {[1, 2, 3].map((item) => (
                <div key={item} className="h-36 animate-pulse rounded-2xl border border-[#e2e6de] bg-white" />
              ))}
            </div>
          ) : (
            <div className="space-y-4">
              ...
              <Section title="Playback context">
                <DetailRow label="Site" value={detail.site ? `${detail.site.name} · ${detail.site.domain}` : "—"} />
                <DetailRow label="Asset" value={detail.asset?.title || "—"} />
              ...
              <Section title="Viewer & device">
                <DetailRow
                  label="Viewer"
                  value={detail.viewer?.displayLabel || detail.viewer?.externalUserId || "—"}
                />
                <DetailRow label="Viewer ID" value={detail.viewer?.externalUserId || "—"} mono />
              ...
              <Section title="Request context">
                ...
                <DetailRow label="User agent" value={detail.session?.userAgent || metadata.userAgent || "—"} mono />
              </Section>
...
export default function SecurityPage() {
  ...
  const critical = useMemo(
    () => events.filter((event) => ["critical", "high"].includes(String(event.severity).toLowerCase())).length,
    [events],
  );
            ...
            {events.map((event) => (
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="mt-1 font-mono text-[11px] text-[#87917f]">risk {event.riskScore}</p>
                </div>
                <StatusPill status={event.severity} />
