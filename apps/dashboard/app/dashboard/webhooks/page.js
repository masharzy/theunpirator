...
export default function WebhooksPage() {
  ...
  const loadDeliveries = useCallback(
    async (endpoint, targetPage = page, targetStatus = status) => {
      if (!endpoint) return;
    ...
    <div className="space-y-8">
      ...
      <form onSubmit={createEndpoint} className="rounded-2xl border bg-white p-5 sm:p-6">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          HTTPS only. Each endpoint gets its own signing secret.
        </p>
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          ...
          <label className="grid gap-2 text-sm font-medium">
            ...
            <Input
              ...
              placeholder="https://example.com/webhooks/unpirator"
              required
            />
          </label>
        </div>
      ...
      {secret && (
        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <Button variant="outline" onClick={() => navigator.clipboard.writeText(secret)}>
              Copy
            </Button>
          </div>
        </div>
      ...
      {(error || message) && (
        <div
          ...
          className={`rounded-xl border px-4 py-3 text-sm ${error ? "border-red-200 bg-red-50 text-red-700" : "bg-slate-50"}`}
        >
          {error || message}
        </div>
      )}
        ...
        {items.length === 0 ? (
          ...
          <div className="space-y-4">
            {items.map((endpoint) => (
              <article
                key={endpoint.id}
                className="rounded-2xl border bg-white p-5 transition hover:border-slate-300 hover:shadow-sm"
              >
                <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                  ...
                  >
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span
                        className={`rounded-full border px-2 py-0.5 text-xs font-medium ${endpoint.enabled ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-slate-200 bg-slate-50 text-slate-600"}`}
                      >
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {(endpoint.events || []).map((event) => (
                        <span key={event} className="rounded-full bg-slate-100 px-2 py-1 text-xs">
                  ...
                  <div className="flex flex-wrap gap-2 xl:justify-end">
                    <Button size="sm" variant="outline" onClick={() => openDeliveries(endpoint)}>
                      Deliveries
                    </Button>
                    <Button
                      size="sm"
                      ...
                      onClick={() =>
                        act(
                          endpoint,
                          "test",
                          () => api(`/v1/webhooks/${endpoint.id}/test`, { method: "POST" }),
                          `Test delivery sent to ${endpoint.name}.`,
                        )
                      }
                    >
                      Send test
                    ...
                    <Button
                      ...
                      onClick={() =>
                        act(
                          ...
                          "toggle",
                          () =>
                            api(`/v1/webhooks/${endpoint.id}`, {
                              method: "PATCH",
                              body: JSON.stringify({ enabled: !endpoint.enabled }),
                            }),
                          endpoint.enabled ? "Webhook paused." : "Webhook enabled.",
                        )
                      }
                    >
                    ...
                    <Button
                      ...
                      onClick={() =>
                        act(
                          ...
                          "secret",
                          () =>
                            api(`/v1/webhooks/${endpoint.id}/rotate-secret`, { method: "POST" }),
                          `Signing secret rotated for ${endpoint.name}.`,
                        )
                      }
                    >
                      Rotate secret
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        return act(
                          endpoint,
                          "delete",
                          () => api(`/v1/webhooks/${endpoint.id}`, { method: "DELETE" }),
                    ...
                    {confirmDelete === endpoint.id && (
                      <Button size="sm" variant="outline" onClick={() => setConfirmDelete("")}>
                        Cancel
                      </Button>
                    )}
                  </div>
      ...
      {selected && (
        <div className="fixed inset-0 z-50 bg-black/20" onMouseDown={() => setSelected(null)}>
          <aside
            className="ml-auto h-full w-full max-w-2xl overflow-y-auto border-l bg-white p-5 shadow-xl sm:p-6"
                  ...
                  <article key={delivery.id} className="rounded-xl border p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-1 text-xs text-muted-foreground">
                          {when(delivery.createdAt)} · {delivery.attempts} attempt
                    ...
                    {delivery.status === "pending" && (
                      <p className="mt-3 text-xs text-muted-foreground">
                        Next attempt: {when(delivery.nextAttemptAt)}
                      </p>
                    )}
